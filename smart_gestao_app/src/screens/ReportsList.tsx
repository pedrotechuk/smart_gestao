...
export default function RelatosScreen({ navigation, route }: Props) {
  ...
  const carregarRelatos = async () => {
    try {
      setLoading(true);
      const response = await axios.get<Relato[]>(`${BASE_URL}/users`, {
  ...
  useEffect(() => {
    carregarRelatos();
    saveLastRoute("Relatos", { token });
  }, []);

  // Abrir modal
  const confirmarExclusao = (relato: Relato) => {
    setRelatoSelecionado(relato);
    setModalVisible(true);
  };

  // Excluir relato
  const excluirRelato = async () => {
    if (!relatoSelecionado) return;

    try {
      await axios.delete(`${BASE_URL}/users/${relatoSelecionado.id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      Toast.show({
        type: "success",
        text1: "Relato excluído",
        text2: `${relatoSelecionado.username} foi removido.`,
      });
  ...
  const renderItem = ({ item }: { item: Relato }) => (
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <Text style={styles.btnCriarText}>+ Novo Relato</Text>
      </TouchableOpacity>
      ...
      <Modal isVisible={modalVisible}>
        <View style={styles.modalContent}>
          <Text style={styles.modalTitle}>Excluir Relato?</Text>

          <Text style={styles.modalMessage}>
            Tem certeza que deseja excluir{" "}
            <Text style={{ fontWeight: "bold" }}>
              {relatoSelecionado?.username}
            </Text>
            ?
          </Text>
